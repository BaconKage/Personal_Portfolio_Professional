...
async function main(){
  ...
  const mod=new Module(file,module);mod.filename=file;mod.paths=module.paths;mod._compile(code,file);
  fs.mkdirSync('public/art',{recursive:true});
  for(const scene of ['hero','mygym','vanicert','firstdrop-ai','bhashabuddy','posture-engine']){
    let svg=renderToStaticMarkup(React.createElement(mod.exports.default,{scene}));
    svg=svg.replace('<svg ','<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="800" ');
    await sharp(Buffer.from(svg)).webp({quality:88}).toFile(`public/art/${scene}.webp`);
  }
